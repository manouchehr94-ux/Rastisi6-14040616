...
**Additional bounded Task2 scope:** B/services/r4_mutation_service.py, only the Brand enable preflight in `_apply_section_update_settings`, and B/tests/test_r4_mutation_api.py plus B/tests/test_render_service.py. Destination capability needs store context: use the existing destination shape/URL validator and `apps/content/services.py:resolve_destination_setting` on trusted persisted destination at the existing store-scoped mutation/inspector boundaries, before save/history/revision. Do not add database access to the pure schema cleaner or alter lifecycle services. Inspector filtering and mutation rejection must apply the same two-part rule; no new generic capability framework or destination editor. Existing legacy/template destination authoring remains intact.
...
- [ ] Declare Brand's variant-specific supported settings using existing metadata. Grid/carousel support show_view_all only when the trusted current destination validates and resolves to a non-none URL for this store; beauty_tabs never supports it. Filter the R4 inspector accordingly. Preserve dormant stored show_view_all/destination on variant switch and unrelated edits. Reject explicit enabling when either condition fails; do not reject a variant-only switch because current stored show_view_all is true. Preserve legacy/template destination authoring and do not condition its availability on an already-populated destination. After variant change reload the existing inspector endpoint so controls reflect server state. Raw destination/capability/href fields cannot grant capability through an unsupported R4 patch.
- [ ] Add six explicit RED/GREEN cases to existing schema/inspector/mutation/render tests: (a) grid with valid own-store destination exposes control, enables successfully and renders the resolved anchor; (b) grid with absent/none/invalid destination rejects enable; (c) beauty_tabs with valid destination rejects enable; (d) supporting variant→beauty_tabs preserves stored enabled value and destination while hiding control/anchor; (e) beauty_tabs→supporting variant restores compatible effective value/anchor; (f) spoofed destination/capability payload and malformed external URL, missing/inactive/foreign targets never grant capability. Repeat supporting cases for carousel. For every rejected mutation snapshot settings, edit_revision and history count and prove all unchanged. Distinguish RED cases exposing current bugs from already-GREEN characterizations; record observed outcomes without manufacturing failures. Reuse existing validators/resolver, no second destination authority.
...
- [ ] Run the added V02 cases with `python manage.py test apps.storefront_builder.tests.test_r4_mutation_api apps.storefront_builder.tests.test_render_service --noinput -v2` in addition to the schema/inspector command below.
...
**Browser:** Brand grid→beauty_tabs→carousel with valid destination, then absent destination fixtures through existing legacy/template paths and invalid persisted-state fixtures through the disposable test harness (never bypass validation in real authoring). Verify visible control truth and rendered anchor href/presence together, dormant value recovery, rejected enable with unchanged revision/history, and unchanged title/source. Task3 repeats this under Preview/Public. **Review:** preserved authority, no client marker injection, dormant settings distinction, no unrelated family changes. **Evidence:** task2_brand_contract.md. **Commit:** `fix: preserve brand variant intent and supported controls`. **STOP:** authority/lifecycle redesign needed or global conditions. **Next readiness:** Brand settings/resource/capability contract GREEN before any Collection production work.
...
- [ ] Extend existing test_page_shell/test_g23_builder_public_content_appearance and test_section_registry (bounded additional test file) with an exact registry-set assertion and parameterized Brand placement on home, product_detail, listing, collection, search and cart. Request each real Public route in the inventory with a published fixture, assert correct page dispatch, Brand presence, shell, ordered CSS/JS assets and inline runtime identities. Require Builder CSS and shared HTMX/Alpine exactly once. Prove Listing/Search E3 equivalence for both page types; do not equate Collection with E3 solely because CSS matches. Run `python manage.py test apps.storefront_builder.tests.test_section_registry apps.storefront_builder.tests.test_page_shell apps.storefront_builder.tests.test_g23_builder_public_content_appearance --noinput -v2`.
- [ ] Before Brand gate, browser-smoke Brand on every distinct E1–E5 envelope at1440/390/768 using disposable Product/Collection route fixtures and a nonempty Search query. Inventory URLs identify routes; use actual fixture slugs. One E3 Listing smoke may cover Search only with the per-type equivalence assertions. Prove required CSS/runtime, computed layout, decoded images, no Home-only dependency, stable asset counts after harness wrapper replacement and no document overflow; compare Preview/Public semantics for matching resources/settings. This adds full-page asset proof on Listing/Search, not their deferred A04 fragment redesign. Any missing envelope blocks Brand PASS.
- [ ] Repeat all six V02 cases from Task2 in renderer tests and browser evidence: control visibility/actionability must agree with resolved anchor presence/href, including valid destination roundtrip through beauty_tabs and absent/invalid destination rejection. Snapshot mutation state for rejected enables. No generic destination editor.
...
**Additional bounded Task5 scope:** existing test_section_registry.py, test_page_shell.py and test_g23_builder_public_content_appearance.py; C/templates/catalog/home_visual.html, product_list.html, product_detail.html and apps/cart/templates/cart/cart_detail.html only for loading existing required Builder styles if a new Collection-specific omission is demonstrated. No runtime/business rewrite. Reuse Task3's canonical stylesheet/envelope mechanism.

- [ ] Repeat Task3's exact six-page registry/dispatch/pilot-presence/asset/shell assertions for collection_tiles, run the same three-module command, and browser-smoke Collection on every distinct E1–E5 envelope at1440/390/768. Assert Listing/Search equivalence separately; Collection detail's different runtime prevents E3 deduplication. Verify CSS/runtime exactly once, computed layout, media, no Home-only dependency, replacement asset stability, no document overflow and applicable Preview/Public semantics. Extend existing catalog integration/public-view tests to characterize E6 Collection index as a direct listing with no pilot render-items projection, and capture its companion shell smoke; do not add a new Builder placement there. Any unproven allowed pilot envelope blocks Collection PASS before shared hardening.
...
Mandatory A06 matrix: inventory E1–E5 for each pilot, all three viewports; automated assertions for all six declared page types. Reuse Task3/5 evidence only if it covers the final implementation; rerun affected envelopes after any shared edit. Deduplicate browser coverage only when actual CSS/JS order and inline runtime identity match, currently E3 Listing/Search. Maintain a ledger linking each family/page type to automated result, distinct envelope, screenshot and metrics; E6 is separate companion evidence. For every pilot envelope verify component CSS, HTMX/Alpine exactly once, computed layout, resolved media, no Home-only reliance, no duplicate assets after repeated replacement, no document overflow and matching Preview/Public semantics where applicable. Keep real Cart HTMX and harness projections labeled separately. No new A04 Listing/Newsletter work. A missing row or failed assertion keeps A06 PARTIAL/OPEN and blocks certification.
...
- [ ] Audit the six-page/two-family A06 ledger against the actual registry and final rendered envelopes, including Task3/5 per-page assertions and Task7 E1–E5 browser proof. Require direct distinct-envelope proof or automated assertions plus proof of an identical shared envelope for every allowed pilot page type. E6 companion evidence does not substitute for E4 Collection detail. Any unproven allowed pilot envelope means A06 PARTIAL/OPEN and no full family PASS. Only the two Phase3 pilot families may be certified; global/all-family A06 remains outside scope. Verify the six-case V02 control/anchor/destination/dormant-state evidence as an explicit exit item.
...
|CSS/JS/A06/media, every allowed pilot page type|T3 Brand/T5 Collection automated six-page assertions; E1–E5 distinct-envelope browser proof; T6 revalidation,T7 ledger,T8 conditional pilot-only closure|
...
|V01|T1,T2,T4,T6|
|V02 variant AND trusted destination; dormant roundtrip|T2 six RED/GREEN cases and atomic rejection; T3 Preview/Public visible-control and rendered-anchor truth; T8 exit audit|
