...
**Revision note (Architect repair round)**: the Bottom Navigation row in §5 below was corrected after direct re-reading of `r4_mutation_service.py::_apply_footer_update` found the initial "just add it to the allowlist" framing was under-scoped. Hero/Product View/Card/Badge's "PARTIAL" classification is retained here as a factual description of current code state, but is **not** to be read as a defect — the binding W5 Builder-vs-Lab decision (see the main plan §7) treats their Design-Lab-only reachability as an intentional product boundary, not a gap. See `gap_matrix.md` for the corrected gap framing.

**Revision note (Final Architecture Correction round)**: §1 Q4/Q11 below (Undo/Redo location) and the R3/R4 discussion elsewhere in this document should be read together with `authority_map.md` §2c's corrected route classification: `storefront_history` (read-only) and `storefront_restore` (mutating, Draft-replacing, no stale-write check) are two distinct capabilities, not one "restore/history" capability — and `storefront_apply_industry_layout` is likewise confirmed mutating with the identical Draft-replacement pattern as `storefront_restore`. Neither Restore nor Industry-Layout-Apply may be treated as a safe unconditional exception under the single-active-write-surface policy; see the main plan §5/§8 (W5A) for the corrected Class A/B/C convergence plan.
...
| **Mega Menu** | **RESERVED/COMPATIBILITY STATE (binding disposition, not a defect)** | Registry has exactly **one** component (`mega_menu.none.v1`) — no second option exists. No renderer consumes `component("mega_menu")` anywhere. The *real* mega-menu UI exists but is owned by specific **Header** variants, not by an independently switchable `mega_menu` family. No selector in editor.html. **Binding W5 disposition**: not advertised as an independent merchant control; not built out in W5; any future independent Mega Menu work must reuse the existing Header/navigation architecture rather than create a second navigation authority. |
| **Hero** | **PARTIAL (Design-Lab-only by binding product decision, not a defect)** | Full registry (13 variants) + renderer + Design-Lab wiring + tests. No persistent, always-on merchant selector in the editor's Global Design panel — reachable via Design Lab or whole-template Apply. (A *different* token, `hero_style`, has a UI selector but is a separate structural setting, not the `hero` family.) **The W5 Builder-vs-Lab decision explicitly keeps this Design-Lab-only for W5** — the merchant already has access through Design Lab; promotion is a possible future UX study, not a W5 gap. |
| **Layout/Composition** | **RESERVED/INERT (binding disposition, not a defect)** for the typed `layout` family (registered, 9 variants, but **zero render consumers found** — selecting a different `layout.*` key has no traced visual effect). A **separate, genuinely COMPLETE** system — per-container `layout_key` (2/3/4-column rows) — is real, wired, tested, and undoable, and already serves the actual merchant composition need. **Binding W5 disposition**: do not build a second store-wide composition renderer merely to activate the typed family; do not advertise it as functioning; preserve existing manifest/recipe compatibility as-is. |
| **Product View** | **PARTIAL (Design-Lab-only by binding product decision, not a defect)** | Same shape as Hero — full registry (7 variants) + renderer + Design-Lab wiring, no persistent selector; kept Design-Lab-only for W5 by explicit decision |
| **Product Card** | **PARTIAL (Design-Lab-only by binding product decision, not a defect)** | Rich registry (19 variants) + renderer + tests; family-level variant kept Design-Lab-only for W5 by explicit decision. Adjacent style toggles (shadow/hover/crossfade/zoom) **do** have real dedicated UI and could be mistaken for full card-family control — worth a documentation note, not a code change. |
| **Badge/promo treatment** | **PARTIAL (Design-Lab-only by binding product decision, not a defect)** | Thin registry (2 variants: none/sale) but real render wiring + tests; kept Design-Lab-only for W5 by explicit decision |
...
| **Mobile Bottom Navigation** | **PARTIAL — real gap, corrected technical scope** | Real registry (7 variants, `global_region_registry.GLOBAL_MOBILE_NAV_REGION`) + real renderer (both preview and public), but three separate things are missing, not just an allowlist entry: (1) `mobile_nav_variant` is absent from `r4_mutation_service.py::_FOOTER_UPDATE_ALLOWED_PATCH_KEYS`; (2) **even if allow-listed, `_apply_footer_update()` has no code path that copies `patch["mobile_nav_variant"]` into `candidate["mobile_nav_variant"]`** — confirmed by direct read: `candidate = dict(draft.effective_footer_config())` only seeds the *existing* value, and no `if "mobile_nav_variant" in patch:` branch exists, so a submitted change would currently be silently dropped even with the key allow-listed; (3) `r4_views.py::_build_global_design_context()` builds `header_variants`/`footer_variants` via `global_region_registry.list_global_variants()` but has no equivalent `mobile_nav_variants` call, so the R4 UI has no choice list to render a selector from, even though `GLOBAL_MOBILE_NAV_REGION` already exists and `list_global_variants()` is generic. A selector exists **only in the legacy (non-canonical) footer editor partial** (`footer_panel.html:29-32`, fed by the legacy view's own `mobile_nav_variants` context, built separately from R4's). The underlying sync target, `appearance_authority_service.apply_footer_variant()`, **already accepts and owns both `footer_variant` and `mobile_nav_variant` in one call** — so no change is needed there. See `gap_matrix.md` for the full A-F corrected fix scope. |
