...
## Correction from the Step-13 independent review pass

The original per-family audit undercounted registered variants for three families whose registry
entries split a "named" tuple and a trailing `_A8_*_VARIANTS` spread tuple across two locations in
`global_region_registry.py`/`section_registry.py` — the first read stopped before the trailing tuple.
Corrected below: **HDR Header is 22 wired variants (not 10, and none are orphaned)**, **FTR Footer is 16
wired variants (not 8)**, **PC Product Card is 22 presentation styles (not 16)**. The real remaining HDR
gap is that variant *selection* is still a legacy config key, not an R4 `SettingsSchema` — not missing
variants. This correction also removed a now-invalid implementation-plan work item (see the Phase-5
...
|---|---|---|---|---|---|---|---|---|---|
| 1 | HDR Header | EXISTS BUT NEEDS REPAIR-EXPANSION | **Corrected count (verified in review pass): 22 wired variants** — 10 named `GlobalVariantDefinition`s (`global_region_registry.py:184-259`) plus `_A8_HEADER_VARIANTS` (12 more: editorial_row, marketplace_search, centered_brand, floating_compact, compact_drawer, promo_bar, community_shortcuts, overlay_transparent, editorial_masthead, compact_menu, category_tabs, playful_canopy — `:267-283`), all spread into `GLOBAL_HEADER_REGION.variants` at `:285-300`. **None are orphaned** — an earlier pass of this table read only the named-variant range and missed the trailing spread tuple; corrected here | Rendered on every route; 12/50 recipes use a Header ref | `header_config` composable: 6 toggles + up to 6 ordered extra blocks (`layout_service.py:113`, Phase 8 P0-3). The repair gap that remains is real: **variant choice itself is a legacy config key (`header_variant`), not a dedicated R4 `SettingsSchema`** — a merchant picks it through the legacy config surface, not a schema-validated R4 field | VERIFIED GOOD (22 wired variants; mobile-overflow bug fixed in Phase 8) | VERIFIED GOOD (site-wide `dir="rtl"`) | LIKELY GAP — partial aria coverage on the originally-audited subset, not independently re-verified across all 22, no confirmed keyboard-trap/focus proof | Yes — core, heavy use; the real remaining gap is an R4 schema for variant selection, not missing variants |
...
| 18 | PC Product Card | EXISTS & REUSE | 22 presentation styles (`CARD_STYLE_CHOICES`, `section_registry.py:890-899` — corrected from an earlier "16" count in the same read-range-truncation pattern as HDR/FTR above), one shared `product_card.html` dispatcher | Universal | R4 YES (Phase 8 P0-2): per-section card settings across 9 section types (brand/price/badge/wishlist/quick-add visibility, image ratio, columns 2-8) | VERIFIED GOOD | VERIFIED GOOD | UNKNOWN | Yes |
...
| 50 | FTR Footer | EXISTS & REUSE | **Corrected count (verified in review pass): 16 wired variants** — 8 named `GlobalVariantDefinition`s plus `_A8_FOOTER_VARIANTS` (8 more: minimal, marketplace_columns, editorial_wordmark, brand_story, bold_columns, centered, app_download, playful_wave — `global_region_registry.py:366-376`), spread into `GLOBAL_FOOTER_REGION.variants` at `:380-392`. Already exceeds the 50-Template DNA Charter's own "at least 10" target with no new work needed | 8/50 refs | `footer_config`: 9 toggles + up to 4 ordered extra blocks (Phase 8 P0-4) | VERIFIED GOOD | VERIFIED GOOD | UNKNOWN | Yes |
