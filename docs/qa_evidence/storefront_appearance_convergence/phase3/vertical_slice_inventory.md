...
D. Partly: unknown R4 fields reject; invalid stored variants fall back to grid; legacy column controls are hidden using columns_visual. But beauty_tabs still receives exposed show_view_all, which it ignores. Existing typography is not a supported Brand local control.
...
|V02 P2 Brand|BRAND_CAROUSEL_SCHEMA show_view_all vs beauty_tabs template branch; destination absent R4 schema|Control promises unsupported rendering; capability truth and hidden preserved values|T2–T3; test_r4_inspector/test_r4_settings_schema; UI control/anchor assertions; not deferred|
