...
| Container/Cell/Row composition (add/settings/layout/move/remove, cell add-section/clear, row layout) | Pre-Task-10 remediation closed `section.move_to_cell` (arbitrary/non-adjacent placement) and the `container.update_settings` mutation type's backend allowlist (gap/mobile_mode/vertical_align/height_mode/background_*). **Independent review (Gap 4) found the backend claim overstated the merchant-facing reality**: `container.update_settings` accepted `background_mode`/`background_color`/`background_pattern` from day one, but `r4/editor.html` rendered no control for any of them — a merchant on R4 had no way to reach a capability the mutation type nominally supported. Fixed this session: `r4/editor.html`/`r4_editor.js` now render and wire background_mode + background_color (sent together — see the code comment for why a single-key patch silently reverts to "transparent"), reusing the legacy form's own fixed `background_pattern` value (never a new pattern-choice UI) | **THIN NON-AUTHORITATIVE ADAPTER** — R4 now has full functional parity (verified: field-reachable, not just backend-accepted) and is the primary path; the legacy views are redundant, still-reachable only as part of the same `editor.html` shell kept live for restore/history + industry-layout-presets | Task 7, Pre-Task-10 remediation, Pre-Task-10 final remediation (independent review fix) |
